...
def is_account_count_equal_to(context, count):
    response = requests.get(URL + "/api/accounts")
    accounts = response.json()
    assert len(accounts) == int(count), f"Expected {count} accounts, but got {len(accounts)}"

@step('Account with pesel "{pesel}" exists in registry')
def check_account_with_pesel_exists(context, pesel):
    response = requests.get(URL + f"/api/accounts/{pesel}")
    assert response.status_code == 200, f"Account with pesel {pesel} does not exist"
...
def field_equals_to(context, pesel, field, value):
    ...
    assert account[field] == value, f"Expected {field} to be {value}, but got {account[field]}"
