...
def test_valid_nip_with_czynny_status(monkeypatch):
    ...
    DummyResponse.json = lambda self: {"result": {"subject": {"statusVat": "Czynny"}}}
...
def test_invalid_nip_not_found_raises_error(monkeypatch):
    ...
    DummyResponse.json = lambda self: {"result": {"subject": None}}
    monkeypatch.setattr(requests, "get", lambda url: DummyResponse())
...
def test_nip_with_non_czynny_status_raises_error(monkeypatch):
    ...
    DummyResponse.json = lambda self: {"result": {"subject": {"statusVat": "Zwolniony"}}}
    monkeypatch.setattr(requests, "get", lambda url: DummyResponse())
    monkeypatch.delenv("BANK_APP_SKIP_MF_CHECK", raising=False)
    with pytest.raises(ValueError, match="Company not registered!!"):
...
def test_get_express_fee(monkeypatch):
    monkeypatch.setenv("BANK_APP_SKIP_MF_CHECK", "1")
    acc = CompanyAccount("Test Sp. z o.o.", "1234567890")
    assert acc.get_express_fee() == 5.0


def test_submit_for_loan(monkeypatch):
    monkeypatch.setenv("BANK_APP_SKIP_MF_CHECK", "1")
    acc = CompanyAccount("Test Sp. z o.o.", "1234567890")
    assert acc.submit_for_loan(1000) == False


def test_take_loan_invalid_amount(monkeypatch):
    monkeypatch.setenv("BANK_APP_SKIP_MF_CHECK", "1")
    acc = CompanyAccount("Test Sp. z o.o.", "1234567890")
    assert acc.take_loan("invalid") == False


def test_take_loan_negative_amount(monkeypatch):
    monkeypatch.setenv("BANK_APP_SKIP_MF_CHECK", "1")
    acc = CompanyAccount("Test Sp. z o.o.", "1234567890")
    assert acc.take_loan(-100) == False


def test_take_loan_insufficient_balance(monkeypatch):
    monkeypatch.setenv("BANK_APP_SKIP_MF_CHECK", "1")
    acc = CompanyAccount("Test Sp. z o.o.", "1234567890")
    acc.balance = 100
    assert acc.take_loan(1000) == False


def test_take_loan_missing_history_entry(monkeypatch):
    monkeypatch.setenv("BANK_APP_SKIP_MF_CHECK", "1")
    acc = CompanyAccount("Test Sp. z o.o.", "1234567890")
    acc.balance = 10000
    assert acc.take_loan(1000) == False
...
def test_take_loan_success(monkeypatch):
    ...
    acc = CompanyAccount("Test Sp. z o.o.", "1234567890")
    acc.balance = 10000
    acc.history.append(-1775)
    assert acc.take_loan(1000) == True
    assert acc.balance == 11000


def test_send_history_via_email(monkeypatch):
    monkeypatch.setenv("BANK_APP_SKIP_MF_CHECK", "1")
    ...
    monkeypatch.setattr("src.company_account.SMTPClient", lambda: DummySMTPClient())
